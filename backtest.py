...
TEST_SUITE = [
    ...
    {
        "name": "Indian Constitution - Part 1",
    ...
    {
        "name": "Indian Constitution - Part 2",
        "document_url": "https://hackrx.blob.core.windows.net/assets/indian_constitution.pdf?sv=2023-01-03&st=2025-07-28T06%3A42%3A00Z&se=2026-11-29T06%3A42%3A00Z&sr=b&sp=r&sig=5Gs%2FOXqP3zY00lgciu4BZjDV5QjTDIx7fgnfdz6Pu24%3D",
        "questions": [
...
async def send_request(client: httpx.AsyncClient, test_case: dict, test_num: int):
    """Sends a single request and prints the result."""
    
    print(f"--- [STARTING] Test Case {test_num}: {test_case['name']} ---")
    ...
    try:
        response = await client.post(API_ENDPOINT, json=payload, timeout=180.0)
        response.raise_for_status() # Raise exception for 4xx/5xx errors
        
        duration = time.time() - start_time
        
        print(f"--- [✅ SUCCESS] Test Case {test_num}: {test_case['name']} completed in {duration:.2f} seconds ---")
        print(json.dumps(response.json(), indent=2))
        print("-" * 60)

    except httpx.HTTPStatusError as e:
        duration = time.time() - start_time
        print(f"--- [❌ FAILURE] Test Case {test_num}: {test_case['name']} failed with status {e.response.status_code} in {duration:.2f} seconds ---")
...
async def main():
    ...
    print("--- Starting Hackathon Concurrent Test Simulation ---")
    
    async with httpx.AsyncClient() as client:
        tasks = [send_request(client, test_case, i + 1) for i, test_case in enumerate(TEST_SUITE)]
